...
USER_NAME = os.environ['USER_NAME'] # 'mazbauddinsaif'

BIRTHDAY = datetime.datetime(2005, 3, 16)
...
def recursive_loc(owner, repo_name, data, cache_comment):
    ...
    Iterative (not recursive) so repos with tens of thousands of commits don't blow the stack
    """
    ...
    while True:
        ...
        if request.json()['data']['repository']['defaultBranchRef'] == None: # repo is empty
            return 0
        history = request.json()['data']['repository']['defaultBranchRef']['target']['history']
        for node in history['edges']: # only adds the LOC value of commits authored by me
...
def total_commits_grs_style():
    """
    Replicates github-readme-stats' 'Total Commits' formula (include_all_commits
    + count_private): all-time public commits from the search API, plus
    private/restricted contributions from the last 365 days — so this card and
    the github-readme-stats card show the same number.
    """
    public_commits = None
    for attempt in range(5):
        request = requests.get(f'https://api.github.com/search/commits?q=author:{USER_NAME}&per_page=1', headers=HEADERS)
        if request.status_code == 200:
            public_commits = int(request.json()['total_count'])
            break
        if request.status_code in (403, 429, 502, 503) and attempt < 4:
            time.sleep(2 * (attempt + 1))
            continue
        raise Exception('total_commits_grs_style() has failed with a', request.status_code, request.text, QUERY_COUNT)
    query = '''
    query($login: String!) {
        user(login: $login) {
    ...
    query_count('graph_commits')
    request = simple_request(total_commits_grs_style.__name__, query, {'login': USER_NAME})
    return public_commits + int(request.json()['data']['user']['contributionsCollection']['restrictedContributionsCount'])
...
if __name__ == '__main__':
    """
    mazbauddinsaif profile README builder
    ...
    commit_data, commit_time = perf_counter(total_commits_grs_style)
